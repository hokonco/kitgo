...
func Test_client_smtp(t *testing.T) {
	...
	t.Run("ReverseLookup", func(t *testing.T) {
		t.Parallel()
		wrap, mock := kitgo.SMTP.Test()
		times := 7

		t.Run("err validate", func(t *testing.T) {
			Expect(wrap.ReverseLookup(ctx, to, to)).To(HaveOccurred())
			Expect(wrap.ReverseLookup(ctx, mail.Address{}, to)).To(HaveOccurred())
			Expect(wrap.ReverseLookup(ctx, from, mail.Address{})).To(HaveOccurred())
		})
		...
		t.Run("err lookupMX", func(t *testing.T) {
			...
			Expect(wrap.ReverseLookup(ctx, from, to)).To(HaveOccurred())
		})
		...
		t.Run("ok", func(t *testing.T) {
			...
			smtpClient.EXPECT().Close()
			mock.WithNewSmtpClient(smtpClient, nil)
			Expect(wrap.ReverseLookup(ctx, from, to)).To(BeNil())
		})
