...
func Test_client_crypto(t *testing.T) {
	...
	wrap := kitgo.Crypto.New()
	t.Run("SHA & base64", func(t *testing.T) {
		t.Parallel()
		password := wrap.Nonce(16)
		Expect(len(password)).To(Equal(16))
		Expect(len(wrap.SHA512(password))).To(Equal(64))
		Expect(kitgo.Base64.RawStd().BtoA([]byte("p45$sW0rd"))).To(Equal([]byte("cDQ1JHNXMHJk")))
		Expect(kitgo.Base64.RawStd().AtoB([]byte("cDQ1JHNXMHJk"))).To(Equal([]byte("p45$sW0rd")))
		Expect(wrap.SHA512(password)).To(Equal(wrap.SHA512(password)))
	})
	t.Run("BCrypt", func(t *testing.T) {
		t.Parallel()
		password := wrap.Nonce(16)
		Expect(wrap.BCryptCompareHashAndPassword(wrap.BCrypt(password), password)).To(BeTrue())
	})
	t.Run("SCrypt", func(t *testing.T) {
		t.Parallel()
		password := wrap.Nonce(16)
		Expect(wrap.SCryptCompareHashAndPassword(wrap.SCrypt(password), password)).To(BeTrue())
	})
	t.Run("Argon2I", func(t *testing.T) {
		t.Parallel()
		password := wrap.Nonce(16)
		Expect(wrap.Argon2ICompareHashAndPassword(wrap.Argon2I(password), password)).To(BeTrue())
	})
	t.Run("Argon2ID", func(t *testing.T) {
		t.Parallel()
		password := wrap.Nonce(16)
		Expect(wrap.Argon2IDCompareHashAndPassword(wrap.Argon2ID(password), password)).To(BeTrue())
	})
	t.Run("tls config", func(t *testing.T) {
		t.Parallel()
		conf := wrap.NewTLSConfig("", "", "")
	...
	t.Run("aesgcm", func(t *testing.T) {
		t.Parallel()
		a := wrap.NewAESGCM([]byte(`random-key`))
...
-----END RSA PUBLIC KEY-----`),
	...
	t.Run("ecdsa load", func(t *testing.T) {
		t.Parallel()
		cECDSA, err := wrap.ReadECDSA(strings.NewReader(``), strings.NewReader(``))
		Expect(err).NotTo(BeNil())
		Expect(cECDSA).NotTo(BeNil())

		cECDSA, err = wrap.ReadECDSA(
			strings.NewReader(`
