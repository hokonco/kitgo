...
func (*Client) BtoA(b []byte) []byte { return btoa(b) }

func (*Client) AtoB(b []byte) []byte { return atob(b) }

func (*Client) SHA256(b []byte) []byte { cs := sha256.Sum256(b); return cs[:] }

func (*Client) SHA512(b []byte) []byte { cs := sha512.Sum512(b); return cs[:] }

func (*Client) Nonce(length int) []byte { return nonce(length) }

func (*Client) NewNaCl() *NaCl { return &NaCl{} }

func (*Client) NewAESGCM(password []byte) *AESGCM {
	aesgcm := &AESGCM{}
	if block, err := aes.NewCipher(new(Client).SHA256(password)); err == nil && block != nil {
		aesgcm.AEAD, _ = cipher.NewGCMWithNonceSize(block, block.BlockSize())
	}
	return aesgcm
}
func (*Client) NewRSA(bits int) (*RSA, error) {
...
func (*Client) ReadRSA(key, pub io.Reader) (*RSA, error) {
...
func (*Client) NewECDSA(curv elliptic.Curve) (*ECDSA, error) {
	if curv == nil {
		curv = elliptic.P384()
	}
...
func (*Client) ReadECDSA(key, pub io.Reader) (*ECDSA, error) {
...
func (*Client) NewTLSConfig(certFile, keyFile, dirCache string, hostWhitelist ...string) *tls.Config {
...
func (*Client) BCrypt(password []byte) []byte {
	hash, _ := bcrypt.GenerateFromPassword(password, bcrypt.DefaultCost)
	return hash
}
func (*Client) BCryptCompareHashAndPassword(hashed, password []byte) bool {
	return bcrypt.CompareHashAndPassword(hashed, password) == nil
}
func (*Client) SCrypt(password []byte) []byte {
...
func (*Client) SCryptCompareHashAndPassword(hashed, password []byte) bool {
	n, r, p := 32768, 8, 1
...
func (*Client) Argon2I(password []byte) []byte {
...
func (*Client) Argon2ICompareHashAndPassword(hashed, password []byte) bool {
...
func (*Client) Argon2ID(password []byte) []byte {
...
func (*Client) Argon2IDCompareHashAndPassword(hashed, password []byte) bool {
	t, m, p := uint32(0), uint32(0), uint8(0)
	parts := bytes.Split(hashed, []byte("$"))
	fmt.Sscanf(string(parts[3]), "m=%d,t=%d,p=%d", &m, &t, &p)
	salt, hash := atob(parts[4]), atob(parts[5])
